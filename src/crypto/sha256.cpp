...
std::string Sha256AutoDetect() {
    ...
    const bool ok [[maybe_unused]] = SelfTest();
    assert(ok);

    return ret;
}

const std::string whichSha256Impl = Sha256AutoDetect(); // ensures all function pointers are populated at app startup

} // namespace

const std::string & GetSha256Implementation() { return whichSha256Impl; }
...
Sha256 &Sha256::Write(std::span<const uint8_t> span) {
...
void Sha256::Finalize(std::span<uint8_t, OUTPUT_SIZE> outbuf) {
    uint8_t * const hash = outbuf.data();
